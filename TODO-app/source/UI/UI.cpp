...
void UI::Render()
{
	...
	::RegisterClassEx(&wc);
	const HWND hwnd = ::CreateWindowA(wc.lpszClassName, ("ImGui Standalone"), WS_POPUPWINDOW, 100, 100, 50, 50, NULL, NULL, wc.hInstance, NULL);
	...
	MainFontcfg.GlyphRanges = ranges;
	...
	{
		fScale = 1.7f;
		MainFontcfg.SizePixels = 13 * fScale;

		style.ScaleAllSizes(fScale);
	}

	//io.Fonts->AddFontDefault(&cfg);
	ImFont* MainFont = io.Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\Verdana.ttf", 0, &MainFontcfg, io.Fonts->GetGlyphRangesCyrillic());
	ImFont* LargeArialBoldFont = io.Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\arialbd.ttf", fScale * 24.0f);
	ImFont* MediumArialBoldFont = io.Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\arialbd.ttf", fScale * 15.0f);
